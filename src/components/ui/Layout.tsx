...
import { LayoutDashboard, Video, ShoppingBag, Settings, Menu, User, LogOut, LogIn, Plus, Users, List, Package } from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
    ...
    const navItems = [
        // Regular USER gets: Dashboard, My Videos, My Plans, Buy Plans, Settings
        ...(user?.role !== 'ADMIN' ? [{ icon: LayoutDashboard, label: 'Dashboard', path: '/dashboard' }] : []),
        ...(user?.role !== 'ADMIN' ? [{ icon: Video, label: 'My Videos', path: '/videos' }] : []),
        ...
        <div className="min-h-screen bg-slate-900 flex">
            ...
            <aside className={cn(
                ...
                <div className="h-16 flex items-center justify-center border-b border-white/10">
                    <h1 className={cn("font-bold text-xl bg-gradient-to-r from-blue-400 to-purple-500 bg-clip-text text-transparent", !sidebarOpen && "hidden")}>
                        YT Simulator
                    </h1>
                    {!sidebarOpen && <span className="text-xl font-bold text-blue-500">YT</span>}
                </div>
                ...
                <nav className="p-4 space-y-2">
                    ...
                    {user ? (
                        ...
                        <Link
                            to="/login"
                            className="flex items-center gap-3 px-4 py-3 rounded-xl text-green-400 hover:bg-green-500/10 hover:text-green-300 transition-all"
                        >
                            <LogIn size={20} />
                            <span className={cn(!sidebarOpen && "hidden")}>Login</span>
                        </Link>
                    )}
                </nav>
            </aside>
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                <footer className="p-4 text-center text-xs text-gray-500 border-t border-white/5">
                    DISCLAIMER: This platform is a simulation. No real YouTube views are generated.
                </footer>
            </div>
        </div>
    );
